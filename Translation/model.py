...
def create_padding_mask(seq, pad):
    ...
    # batch_size * max_length_of_batch  ->  batch_size * 1 * 1 * max_length_of_batch
    return seq[:, np.newaxis, np.newaxis, :]


# 只看当前token前面的词，后面的都mask掉为1，增加训练数据
def create_look_ahead_mask(size):  # max_length_of_batch
    mask = torch.triu(torch.ones((size, size)), diagonal=1)
    return mask  # [max_length_of_batch, max_length_of_batch]


def create_mask(inp, targ):
    # 编码padding
    enc_padding_mask = create_padding_mask(inp, pad=0)
    # future token padding
    look_ahead_mask = create_look_ahead_mask(targ.shape[-1]).to(device)  # =>[max_length_of_batch, max_length_of_batch]
    dec_targ_padding_mask = create_padding_mask(targ, pad=0)
    combined_mask = torch.max(look_ahead_mask, dec_targ_padding_mask)  # 结合了2种mask，取并集 =>[batch_size, 1, max_length_of_batch, max_length_of_batch]
    ...
    dec_padding_mask = create_padding_mask(inp, pad=0)  # =>[b,1,1,inp_seq_len] mask=1的位置为pad
